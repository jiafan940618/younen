...
import com.yn.kftService.XmlUtils;
	...
	private PyOrderService pyOrderService;
	 ...
	 /** pc端*/
		...
		public Object doOnline(HttpServletRequest request){
			...
			billOrderVo.setTradeNo("2017072514293934173");
			/** 手机端是微信app支付*/  /** wxApp*/
		...
		BillOrder billOrder =	billorderService.findByTradeNo(billOrderVo.getTradeNo());
		

			 //等于1是余额支付
			if(billOrderVo.getPayWay()==1){
				logger.info("--- ---- ---- ---- ----- ---- --- 进入方法1：");
				return pyOrderService.payBalance(billOrder);
				 //等于3是支付宝支付	
		            ...
		            if (inStream != null) {
		                ...
		                if(resultMap.get("resultCode").equals("0") && resultMap.get("returnCode").equals("0")){
			                	if(resultMap.get("status").equals("02")){
			                	 ...
			                	 outTradeNo =(String) resultMap.get("outTradeNo");
			                	Double amount = Double.valueOf(resultMap.get("amount")) ;
			                	orderService.UpdateOrStatus(outTradeNo, amount);

			                
			                	BillOrderVo billOrderVo = new BillOrderVo();
			                	billOrderVo.setStatus(Integer.parseInt(resultMap.get("status")));
