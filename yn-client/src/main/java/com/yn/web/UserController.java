...
public class UserController {
	...
	public Object save(@RequestBody UserVo userVo) {
		...
		/*
		 * addressText = "松山湖"; email = "nicb@engrossing.cn"; phone =
		 * 13450699433; realName = "刘"; userName = "刘先生";
		 */
	...
	@ResponseBody
	public Object findAll(UserVo userVo,
	...
	public Object updateUser(UserVo userVo, String code4register, HttpSession httpSession) {
		/*
		 * userVo.setId(8L); userVo.setFullAddressText("测试地址");
		 * userVo.setEmail("974426563@163.com"); userVo.setHeadImgUrl(
		 * "http://oss.u-en.cn/img/d0b9fdc2-e45c-4fe2-970e-13fbdde03d15.png");
		...
		if (!newuserVo.getPhone().equals(userVo.getPhone())) {
			...
			if (!code4register.equals(attribute2)
					|| !userVo.getPhone().equals(httpSession.getAttribute("user_phone"))) {
	...
	public Object findOrder(com.yn.model.Page<Order> page, HttpSession httpSession) {
		...
		if (num <= 0) {
			...
			page.setTotal(num % page.getLimit() == 0 ? num / page.getLimit()
					: (num - num % page.getLimit()) / page.getLimit() + 1);
		}
	...
	public Object helloJsp01(com.yn.model.Page<TransactionRecord> page, HttpSession httpSession) {
		/*
		 * page.setUserId(2L); page.setTime_to("2017-10-20");
		 */
		...
		if (total <= 0) {
			...
			page.setTotal(total % page.getLimit() == 0 ? total / page.getLimit()
					: (total - total % page.getLimit()) / page.getLimit() + 1);
		}
	...
	public Object ioshelloJs(TransactionRecordVo transactionRecordVo, HttpSession httpSession) {
		...
		List<TransactionRecord> list = transactionRecordService
				.FindByTransactionRecord(transactionRecordVo.getUserId());
