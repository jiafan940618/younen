...
public class WalletController {
	...
	@ResponseBody
	@RequestMapping(value = "/findWalletByUser", method = { RequestMethod.POST })
	public Object findWalletByUser(UserVo userVo) {
		User user = new User();
		BeanCopy.copyProperties(userVo, user);
		Wallet wallet = walletService.findWalletByUser(user.getId());
		if (wallet == null) {
			walletService.createWallet(user);
			wallet = walletService.findWalletByUser(user.getId());
		}
		return ResultVOUtil.success(wallet);
	}
	...
	@ResponseBody
	public Object findAll(WalletVo walletVo,
