...
public class KFTpayService {
   ...
   //2017092300132528
	...
	//快捷协议代扣协议申请(快捷协议代扣步骤1)
		public static void treatyCollectApply() throws GatewayClientException {
			...
			dto.setOrderNo("201706230000002");//订单号同一个商户必须保证唯一
			dto.setTreatyType("11");//11借计卡扣款  12信用卡扣款
			dto.setNote("协议说明");//协议简要说明，可空
			dto.setStartDate("20151010");//协议生效日期,日期格式yyyyMMdd
			dto.setEndDate("20181010");//协议失效日期,日期格式yyyyMMdd
			dto.setHolderName("嘉凡");//持卡人真实姓名
			dto.setBankType("1051000");//银行卡行别，测试环境只支持建行卡
			dto.setBankCardType("1");//0存折 1借记 2贷记
			dto.setBankCardNo("622848231231231232231");//银行卡号
			dto.setMobileNo("13613027433");//银行预留手机号
			dto.setCertificateType("0");//持卡人证件类型，0身份证
			dto.setCertificateNo("443352353223423423113");//证件号码
...
//			dto.setCustCardCvv2("信用卡cvv2");//可空，信用卡扣款时必填
		...
		public static void confirmTreatyCollectApply() throws GatewayClientException {
			...
			dto.setSmsSeq("20170923035725");//协议代扣申请返回的短信流水号
			dto.setAuthCode("145420");
			dto.setHolderName("张三");//持卡人姓名，与申请时一致
			dto.setBankCardNo("622848231231231232231");//银行卡号，与申请时一致
			System.out.println("请求信息为：" + dto.toString());
		...
		//快捷协议代扣(协议代扣步骤3)
		public static void treatyCollect() throws GatewayClientException {
			...
			dto.setOrderNo("2017082109302989566");//订单号同一个商户必须保证唯一
			...
			dto.setBankCardNo("622848231231231232231");//银行卡号，与申请时一致，本次交易中,从客户的哪张卡上扣钱
