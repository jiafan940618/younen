...
public interface OrderDao extends JpaRepository<Order, Long>, JpaSpecificationExecutor<Order> {
  ...
  @Query(value="SELECT o.link_man,o.link_phone,o.address_text,o.server_name,o.order_code, "+
    ...
    String findByStationId(Long orderId);
  	...
  	@Query(value ="SELECT t.`id` t_id ,t.`capacity`,t.`server_name`,t.`order_code`,t.grid_connected_stepa,s.id s_id ,s.`station_name`,t.total_price,t.had_pay_price ,t.status"+
  " FROM t_order t LEFT JOIN station s ON s.`order_id` = t.id "+
  " WHERE t.grid_connected_stepa = 1  AND t.`status` <> 3 AND t.`del` = 0 AND t.user_id = :userId AND s.`status` = 0", nativeQuery = true)
