...
public interface OrderPlanDao extends JpaRepository<OrderPlan, Long>, JpaSpecificationExecutor<OrderPlan> {
    ...
    @Query(value = "SELECT o.order_code,o.construction_status ,p.battery_board_brand,p.battery_board_model,p.inverter_brand,p.inverter_model,a.apo_ids FROM t_order o" 
    ...
    @Query(value = "SELECT s.id,s.server_id,s.user_id,s.station_code,s.capacity,s.link_man,s.status,r.company_name,"
    		+ " a.c_addr,s.order_id FROM station s LEFT JOIN server r ON  s.server_id = r.id "
