...
public interface ElecDataHourDao extends JpaRepository<ElecDataHour, Long>, JpaSpecificationExecutor<ElecDataHour> {
    ...
    @Query("select COALESCE(sum(t.kwh),0) from ElecDataHour t WHERE t.dAddr like CONCAT(?1,'%') AND t.del=0")
    double sumKwh(Long dAddr);
    
    @Query("select COALESCE(sum(t.kwh),0) from ElecDataHour t WHERE t.dAddr like CONCAT(?1,'%') AND t.serverId=?2 AND t.del=0")
    double sumKwh(Long dAddr, Long serverId);
    
    @Query("select COALESCE(sum(t.kwh),0) from ElecDataHour t WHERE t.createDtm>=?1 AND t.createDtm<?2 AND t.dAddr like CONCAT(?3,'%')  AND t.del=0")
    double sumKwh(Date startDtm, Date endDtm, Long dAddr);
    
    @Query("select COALESCE(sum(t.kwh),0) from ElecDataHour t WHERE t.createDtm>=?1 AND t.createDtm<?2 AND t.dAddr like CONCAT(?3,'%') AND t.serverId=?4 AND t.del=0")
    double sumKwh(Date startDtm, Date endDtm, Long dAddr, Long serverId);
    
//    @Query("select COALESCE(sum(t.kwh),0) from ElecDataHour t WHERE t.createDtm>=?1 AND t.createDtm<?2 AND t.dAddr like CONCAT(?3,'%') AND t.stationId=?4 AND t.del=0")
//    double sumKwhByStationId(Date startDtm, Date endDtm, Long dAddr, Long stationId);
    ...
    List<ElecDataHour> findByDAddrAndCreateDtmBetween(Long dAddr, Date start, Date end);
    ...
    List<ElecDataHour> findByServerIdAndDAddrAndCreateDtmBetween(Long serverId, Long dAddr, Date start, Date end);
...
//    List<ElecDataHour> findByStationIdAndDAddrAndCreateDtmBetween(Long stationId, Long dAddr, Date start, Date end);

    @Query("SELECT DATE_FORMAT(create_dtm,'%Y-%m') AS create_dtm, SUM(kwh) AS kwh FROM ElecDataHour t WHERE t.stationId=?1 AND t.dAddr like CONCAT(1,'%') GROUP BY DATE_FORMAT(create_dtm,'%Y-%m')ORDER BY create_dtm ASC")
